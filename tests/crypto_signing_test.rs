...
fn test_sign_and_verify_message() {
    ...
    assert!(!result, "Signature verification should fail with wrong public key");
...
fn test_sign_empty_message() {
    ...
    assert!(!result, "Signature verification should fail with different message");
}
...
fn test_sign_large_message() {
    ...
    let result = crypto::verify_signature(&modified_large_message, &signature, &public_key).unwrap();
    assert!(!result, "Signature verification should fail with modified large message");
}
