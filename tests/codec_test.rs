...
fn test_codec_read_write_request() {
    ...
    let write_result = task::block_on(async {
        codec
            .write_request(&protocol, &mut cursor, request.clone())
            .await
    });
    ...
    let read_result =
        task::block_on(async { codec.read_request(&protocol, &mut read_cursor).await });
...
fn test_codec_read_write_response() {
    ...
    let write_result = task::block_on(async {
        codec
            .write_response(&protocol, &mut cursor, response.clone())
            .await
    });
    ...
    let read_result =
        task::block_on(async { codec.read_response(&protocol, &mut read_cursor).await });
...
fn test_codec_large_data_handling() {
    ...
    // Write the request to the buffer
    let write_result =
        task::block_on(async { codec.write_request(&protocol, &mut cursor, request).await });
