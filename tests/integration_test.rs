...
impl Drop for TestNode {
    fn drop(&mut self) {
        ...
        println!("Cleaning up node with port {} and data dir {}", self.port, self.data_dir);
...
async fn test_peer_discovery_and_transfer() {
    ...
    assert!(transfer_success, "File was not received by node2. Expected at {}", downloaded_file);
...
fn start_node(data_dir: &str) -> TestNode {
    ...
    cmd.args(["run", "--", "start", "--port", &port.to_string(), "--data-dir", data_dir])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
            ...
            for line in reader.lines() {
                if let Ok(line) = line {
                    ...
                    if line.contains("Local peer id:") {
                        if let Some(id) = line.split("Local peer id:").nth(1).map(|s| s.trim().to_string()) {
