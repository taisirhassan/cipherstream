...
pub fn generate_signing_keypair() -> CryptoResult<(Vec<u8>, Vec<u8>)> {
    ...
    let key_pair =
        Ed25519KeyPair::from_pkcs8(pkcs8_bytes.as_ref()).map_err(|_| CryptoError::InvalidKey)?;
...
pub fn sign_message(message: &[u8], private_key: &[u8]) -> CryptoResult<Vec<u8>> {
    let key_pair = Ed25519KeyPair::from_pkcs8(private_key).map_err(|_| CryptoError::InvalidKey)?;
